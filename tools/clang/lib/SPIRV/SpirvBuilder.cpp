...
SpirvExtInstImport *SpirvBuilder::getExtInstSet(llvm::StringRef importName) {
  SpirvExtInstImport *glslSet = module->getExtInstSet(importName);
  if (!glslSet) {
    ...
    glslSet =
        new (context) SpirvExtInstImport(/*SourceLocation*/ {}, importName);
